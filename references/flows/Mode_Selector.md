# Mode Selector

**触发时机**：Phase 0 初始化完成后，用户选择创作模式

---

## 模式选择

```
🎯 爆款网文创作模式

请选择创作模式：

【Fast 快速模式】5 个 Phase
爱好者・极简｜快速开始｜快开写
适合：已有明确想法，想立刻动笔的爱好者

【Professional 专业模式】8 个 Phase
作者・完整但不繁琐｜质量优先
适合：认真创作，追求质量的作者

【Industrial 工业模式】10 个 Phase
团队・最完整・可流水线
适合：长篇创作，团队协作，流水线生产
```

---

## 三模式对比

| 维度 | Fast | Professional | Industrial |
|------|------|--------------|------------|
| **Phase数** | 5 | 8 | 10 |
| **市场调研** | ❌ | ❌ | ✅ |
| **问答深度** | 极简 | 适中 | 完整 |
| **大纲精细度** | 简略 | 完整 | 模块化 |
| **写作模式** | 串行 | 串行+并行 | 串行+并行+Teams |
| **润色深度** | 基础 | 完整 | 工业化QC |
| **包装发布** | 简单 | 完整 | 运营规划 |
| **适用场景** | 快速动笔 | 单人创作 | 团队协作 |

---

## 模式详细说明

### Fast 快速模式（5 Phase）

| Phase | 文件 | 职责 |
|-------|------|------|
| 0 | Fast/Fast0_Initialization.md | 初始化 |
| 1 | Fast/Fast1_Idea_Clarify.md | 想法明确（极简问答） |
| 2 | Fast/Fast2_Quick_Draft.md | 快速起草（串行写作） |
| 3 | Fast/Fast3_Simple_Polish.md | 简单润色（AI味清除） |
| 4 | Fast/Fast4_Final_Validation.md | 最终校验（字数+爽点） |

### Professional 专业模式（8 Phase）

| Phase | 文件 | 职责 |
|-------|------|------|
| 0 | Pro/Pro0_Initialization.md | 初始化 |
| 1 | Pro/Pro1_Core_Clarify.md | 核心明确（类型/金手指/爽点/主角） |
| 2 | Pro/Pro2_World_Character_Setup.md | 世界观与人设 |
| 3 | Pro/Pro3_Outline_Planning.md | 大纲规划 |
| 4 | Pro/Pro4_Full_Draft_Writing.md | 正文撰写 |
| 5 | Pro/Pro5_Polish_Pacing.md | 润色节奏 |
| 6 | Pro/Pro6_Hook_Packaging.md | 钩子包装 |
| 7 | Pro/Pro7_Validation_Release.md | 校验发布 |

### Industrial 工业模式（10 Phase）

| Phase | 文件 | 职责 |
|-------|------|------|
| 0 | Ind/Ind0_Project_Initialize.md | 项目初始化 |
| 1 | Ind/Ind1_Market_Research.md | 市场调研 |
| 2 | Ind/Ind2_Core_Positioning.md | 核心定位 |
| 3 | Ind/Ind3_World_Rule_Setup.md | 世界规则设定 |
| 4 | Ind/Ind4_Character_Standard.md | 人物标准化 |
| 5 | Ind/Ind5_Modular_Outline.md | 模块化大纲 |
| 6 | Ind/Ind6_Team_Writing.md | 团队写作 |
| 7 | Ind/Ind7_Unified_Polish.md | 统一润色 |
| 8 | Ind/Ind8_QC_Validation.md | QC校验 |
| 9 | Ind/Ind9_Release_Operation.md | 发布运营 |

---

## 选择后进入对应模式

**用户选择 Fast** → 进入 `./Fast/Fast0_Initialization.md`

**用户选择 Professional** → 进入 `./Pro/Pro0_Initialization.md`

**用户选择 Industrial** → 进入 `./Ind/Ind0_Project_Initialize.md`
